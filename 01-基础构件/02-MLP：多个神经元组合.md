...
> 📚 **前置知识提示**
> 
> 下面的代码使用了 **Batch Normalization（批归一化）** 和 **Dropout**：
> - **BatchNorm**：让每层的输入分布更稳定，训练更快、更不容易发散
> - **Dropout**：训练时随机关掉一部分神经元，防止模型死记硬背训练数据
> 
> 详细原理将在[第6章：正则化](06-正则化：让模型学会泛化.md)中讲解。现在只需知道它们是帮助模型训练更稳定的技巧，不影响你理解MLP的核心结构。
