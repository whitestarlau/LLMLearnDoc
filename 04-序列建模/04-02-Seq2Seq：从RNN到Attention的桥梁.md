...
### 4.2 Attention：动态组装上下文

Bahdanau et al. (2014) 的核心洞察是：**不要试图用一个固定向量装下所有信息，而是让解码器按需取用**。

#### 核心思想：保存所有状态，动态组合

传统 Seq2Seq 的瓶颈在于编码器只输出**一个**向量。Attention 改变了这一点：

```
传统 Seq2Seq:                      Attention 机制:
┌─────────────────┐                ┌─────────────────────────────┐
│ 输入: 20个词     │                │ 输入: 20个词                 │
│       ↓         │                │       ↓                     │
│ 编码器 LSTM     │                │ 编码器 LSTM（双向）          │
│       ↓         │                │       ↓                     │
│ 只保留最后状态   │                │ 保留所有时间步状态            │
│       ↓         │                │       ↓                     │
│ 1个向量 [c]     │                │ 20个向量 [h₁,h₂,...,h₂₀]    │
...
**关键区别**：编码器不再丢弃中间状态，而是把每个时间步的隐藏状态都保存下来。

#### 解码器如何"组装"上下文

解码器在生成每个词时，都会做三件事：

1. **计算注意力分数**：当前解码状态与每个编码器状态的相关性
2. **归一化为权重**：softmax 得到概率分布
3. **加权求和**：用权重对所有编码器状态做加权平均，得到当前步的上下文向量

```
生成 "The" 时:                    生成 "sat" 时:
┌─────────────────────────────┐  ┌─────────────────────────────┐
│ 权重: [0.7, 0.2, 0.1, ...]   │  │ 权重: [0.1, 0.1, 0.8, ...]   │
│       ↓                      │  │       ↓                      │
│ c₁ = 0.7×h₁ + 0.2×h₂ + ...  │  │ c₂ = 0.1×h₁ + 0.1×h₂ + ...  │
│       ↓                      │  │       ↓                      │
│ 上下文向量 c₁ → 生成 "The"   │  │ 上下文向量 c₂ → 生成 "sat"   │
└─────────────────────────────┘  └─────────────────────────────┘

注意：c₁ 和 c₂ 维度相同（比如都是 256 维），
      但内容不同——是根据当前需要动态组装出来的！
```

#### 为什么这解决了瓶颈问题？

传统方法就像把整本书的内容强行总结成一句话，然后凭这一句话复述全书。Attention 则像翻译时**反复查阅原文**——每翻译一个词，就回头看看原文的哪些部分最相关。

```
翻译 "猫 坐在 垫子 上" → "The cat sat on the mat"

生成 "The": 注意力聚焦 → [猫: 0.7, 坐在: 0.1, 垫子: 0.1, 上: 0.1]
              上下文向量主要包含"猫"的语义
              
生成 "sat": 注意力聚焦 → [猫: 0.1, 坐在: 0.8, 垫子: 0.05, 上: 0.05]
              上下文向量主要包含"坐在"的语义
              
生成 "mat": 注意力聚焦 → [猫: 0.05, 坐在: 0.05, 垫子: 0.85, 上: 0.05]
              上下文向量主要包含"垫子"的语义
```

这样，即使输入很长（50个词、100个词），解码器每一步都能精准定位到最相关的部分，而不会受到信息瓶颈的限制。
