...
我们已经学习了分词和数据准备。现在，让我们动手实践——**从零开始训练一个 GPT 模型**。

这不仅是巩固知识的最佳方式，也是真正理解大模型工作原理的必经之路。当你亲自搭建数据流、构建模型、编写训练循环后，那些曾经抽象的概念都会变得具体而清晰。

本章将带你完成整个流程：从准备数据，到搭建模型，再到训练、保存、生成。你将拥有一个虽小但完整的 GPT。
...
- **上一章**：[19-分词与训练准备](19-分词与训练准备.md)
- **下一章**：[21-BERT：双向理解的力量](21-BERT：双向理解的力量.md)
